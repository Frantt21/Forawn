...
class _MusicPlayerScreenState extends State<MusicPlayerScreen> {
  ...
  void _onPlayPauseChanged() {
    debugPrint('[MusicPlayer] Play pause changed: ${_musicPlayer.isPlaying.value}');
    if (mounted) {
      setState(() {
        // Forzar rebuild para actualizar el botón play/pausa
        debugPrint('[MusicPlayer] Updating UI - isPlaying: ${_musicPlayer.isPlaying.value}');
      });
    }
  }
  ...
  Future<void> _loadMetadata(String filePath) async {
    try {
      ...
      final artist = widget.getText('unknown_artist', fallback: 'Unknown Artist');
  ...
  void _playPrevious() {
    ...
    if (_musicPlayer.position.value.inSeconds >= 3) {
      ...
      if (previousTrack != null) {
        final index = _files.indexWhere((f) => (f as File).path == previousTrack.path);
        if (index >= 0) {
          _playFile(index);
        }
      } else {
  ...
  void _togglePlayPause() {
    debugPrint('[MusicPlayer] Toggle play/pause pressed. Current state: ${_musicPlayer.isPlaying.value}');
